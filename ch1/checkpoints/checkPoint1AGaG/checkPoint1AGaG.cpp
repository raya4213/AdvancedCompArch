...
unsigned int BranchHisLength = 8;
unsigned int BranchTable[256];
...
bool BTB_prediction(ADDRINT ins_ptr)
{
    ...
    index = mask & ins_ptr;
    int prediction;
    prediction = BranchTable[(BranchHis & 0Xff)];
...
{
	int index = BranchHis & 0xFF;

        if (taken)
        {
                if (BranchTable[index] >= 3)
                {
                        BranchTable[index] = 3;
                }
                else

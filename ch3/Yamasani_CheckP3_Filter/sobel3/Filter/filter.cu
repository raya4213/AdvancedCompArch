...
{
    // creating timers
    //cutilCheckError(cutCreateTimer(&GPU_time));
    //cutilCheckError(cutCreateTimer(&CPU_time));
    //cutilCheckError(cutCreateTimer(&hostToDeviceTime));
    //cutilCheckError(cutCreateTimer(&deviceToHostTime));
    ...
    cutStartTimer(deviceToHostTimer);
    cudaMemcpy(out, d_Out, dib.image_size*sizeof(unsigned char), cudaMemcpyDeviceToHost);
    cutStopTimer(deviceToHostTimer);
    
    float getCpuTime = cutGetTimerValue(cpuTimer);
    float getGpuTime = cutGetTimerValue(gpuTimer);
    float getHtoDTime = cutGetTimerValue(hostToDeviceTimer);
    float getDtoHTime = cutGetTimerValue(deviceToHostTimer);
    printf("Printing out the results\n\r");
    printf("cpu time: %0.6f", getCpuTime);
    printf("GPU Time: %0.6f", getGpuTime);
    printf("Host 2 Device Time: %0.6f", getHtoDTime);
    printf("Device 2 Host Time: %0.6f", getDtoHTime); 

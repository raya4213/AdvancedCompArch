...
__global__ void HighBoostFilter(unsigned char* g_DataIn, unsigned char* g_DataOut, int width, int height)
{
  ...
  int x = blockIdx.x*TILE_WIDTH + threadIdx.x ;//- FILTER_RADIUS;             |  ----------------------------------------------------------------------------
    int y = blockIdx.y * TILE_HEIGHT + threadIdx.y ;//- FILTER_RADIUS;        |    int y = blockIdx.y * TILE_HEIGHT + threadIdx.y ;//- FILTER_RADIUS;
                                                                              |
    // Get the Global index into the original image                           |    // Get the Global index into the original image
    int index = y * (width) + x;  
...
int x = blockIdx.x*TILE_WIDTH + threadIdx.x ;//- FILTER_RADIUS;
  ...
  // STUDENT: write code for High Boost Filter : use Sobel as base code for(int dy = -FILTER_RADIUS; dy <= FILTER_RADIUS; dy++) {
   for(int dx = -FILTER_RADIUS; dx <= FILTER_RADIUS; dx++) {
...
g_DataOut[index] = CLAMP_8bit((int)(Pixel + HIGH_BOOST_FACTOR * (unsigned char)(Pixel - sumX / 9)));
}
